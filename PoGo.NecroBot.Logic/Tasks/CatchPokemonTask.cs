    ...
    public static class CatchPokemonTask
    {
            ...
            FortData currentFortData = null, ulong encounterId = 0)
        {
            ...
            if (session.LogicSettings.OnlyCatchHighIvPokemon)
            {
                ...
                {
                    Logger.Write($"Encounter {session.Translation.GetPokemonTranslation(pokemon.PokemonId)} with IV {pokemonIv:#00.00}% at [{pokemon.Latitude}, {pokemon.Longitude}] NOT to catch.", LogLevel.Info);
                    return;
                }
            }
